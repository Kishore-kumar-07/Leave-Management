...
function Nav(props) {
  return (
    ...
    <div className="bg-slate-100  h-full w-[7rem] rounded-xl flex flex-col items-center gap-y-[3rem]  py-[2rem]">
      <Link to ={`/Employee/${props.username}`} className="icon-a icon-hover flex items-center justify-center flex-col gap-y-2 mt-[4rem]">
        <MdDashboard className="text-3xl text-slate-900"/>
        <p className="text-md font-semibold">Dashboard</p>
      </Link>
      <Link to={`/EmpProfile/${props.username}`} className="icon-a icon-hover flex items-center justify-center flex-col gap-y-2">
